...
def getMarketList(req):
    ...
    queryString = {'query': query, 'page': 1, 'size': 15, 'sort': 'accuracy',
                   'x': 126.651542258118, 'y': 37.4514480321002, 'radius': 1500}
    if req.POST.get('type'):
        queryString['category_group_code'] = req.POST.get('type')
    else:
        queryString['category_group_code'] = 'FD6'
    url = "https://dapi.kakao.com/v2/local/search/keyword.json"
    read = requests.get(url, params=queryString, headers={
    ...
    return JsonResponse({'count':len(storeListParse),'list': storeListParse}, json_dumps_params={'ensure_ascii': False})
